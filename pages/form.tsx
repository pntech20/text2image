...
export default function Form({ setImages }: { setImages: (images: string[]) => void }) {
  ...
  return (
    <div className={styles.form}>
      <TextInput onChange={(e) => setDataByKey('prompt', e.target.value)} label="A text description of the desired image(s). The maximum length is 1000 characters." defaultValue={formData.prompt} classNames={classes} size="lg" />
      ...
      <div className={styles.button}>
        {isLoading ? <Loader variant='bars' /> : <Button onClick={() => generateImages(formData)}>Generate</Button>}
      </div>
    </div>
  );
}